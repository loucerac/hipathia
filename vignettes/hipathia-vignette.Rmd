...
author: "Marta R. Hidalgo, Francisco Salavert, Çankut Cubuk, Alicia Amadoz, José Carbonell-Caballero, Joaquín Dopazo"
date: "`r Sys.Date()`"
...
<!-- HiPathia uses -->
We have developed a module which is able to transform KEGG pathway KGML files into SIF files with attibutes.
...
library(devtools)
install_github("martahidalgo/hpAnnot")
install_github("martahidalgo/hipathia")
```

Note that the `devtools` package is a hard dependency: it provides `install_github()`, which enables 
installing packages from GitHub in a manner analogous to that of `install_packages()`. The `devtools` 
package can be installed via `install.packages("devtools")`.
...
```{r, fig.show='hold'}
pathways_only2 <- load.pathways(species = "hsa", pathways.list = c("hsa03320", "hsa04014"))
```
...
Pathways are representaed by graphs and composed by nodes and relations
...
$n$ of the network, the signal value is propagated along the nodes according
to the following rule
\begin{equation}
\label{formula}
 S_n = v\cdot(1-\prod_{s_i \in A}(1-s_i))\cdot\prod_{s_j\in I}(1-s_j)
\end{equation}

\noindent where $A$ is the set of signals arriving to the node from an
activation edge, $I$ is the set of signals arriving to the node from an 
inhibition edge, and $v$ is the normalized value of the current node. 
...
computed using function `node.color.per.differential.expression`. Note that
...
colors_de_hipathia <- node.color.per.differential.expression(results, pathways, 
                                                             sample_group, 
                                                             "Tumor", "Normal", 
                                                             colors = 
                                                                 "hipathia")
...
the top right panel. Arrows pointing up and down lo the left of the names of
